...
* Nyanator メインクラス
*/
type MESSAGES = {BUSY: string, NSFW_FILTERD: string}
class Nyanator {
  /** 
  ...
  * @param {string} usesrMessage - ユーザーからのメッセージ
  ...
  */
  tryLock(lock : GoogleAppsScript.Lock.Lock) {
    if (!lock.tryLock(500)) {
  ...
  */
  checkUserMessage(userId : string, userMessage : string) {
  ...
  * @return {string} 結果文字列
  */
  postTextDataToHuggingFaceAPI(textData : string, huggingFace : HuggingFace) {
  ...
  */
  putBase64JpegFileToDropBox(filePrefix : string, data : string, dropbox : Dropbox) {
    //保存ファイル設定
  ...
  * @return {string} 書き出したJpgeファイルの公開URL
  */
  static modePropertyKey(userId : string) {
